...
import { MinecraftPlugin, MinecraftPluginProvider } from '@/blueprint/extensions/minecraftpluginmanager/MinecraftPluginContainer';
...
export default ({ provider, plugin, className }: Props) => {
    ...
    const installPlugin = () => {
        http.post(`/api/client/extensions/minecraftpluginmanager/servers/${uuid}/minecraft-plugins/install`, {
            provider: provider,
            pluginId: plugin.id,
    ...
    useEffect(() => {
        if (installDialogVisible && !versions.length) {
            http.get(`/api/client/extensions/minecraftpluginmanager/servers/${uuid}/minecraft-plugins/versions`, {
                params: {
                    provider: provider,
                    pluginId: plugin.id,
                },
    ...
    return (
        <>
            ...
            >
                <p>
                    You requested the installation of the plugin &quot;{plugin.name}&quot; from the {provider}{' '}
                    provider. Please select the desired plugin version below.
                </p>
