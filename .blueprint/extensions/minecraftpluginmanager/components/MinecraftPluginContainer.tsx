...
import MinecraftPluginRow from '@/blueprint/extensions/minecraftpluginmanager/MinecraftPluginRow';
...
export default () => {
    ...
    const { data: isLinkedToPolymart, mutate } = useSWR<boolean>(
        ...
        async () => {
            const { data } = await http.get(
                `/api/client/extensions/minecraftpluginmanager/servers/${uuid}/minecraft-plugins/is-linked`
            );
            return data;
        }
    );
    ...
    const {
        ...
        async () => {
            const { data } = await http.get(
                `/api/client/extensions/minecraftpluginmanager/servers/${uuid}/minecraft-plugins`,
                {
                    params: {
    ...
    const handleLinkButton = async () => {
        const { data: redirectUrl } = await http.post(
            `/api/client/extensions/minecraftpluginmanager/servers/${uuid}/minecraft-plugins/link`
        );
        window.location = redirectUrl;
    };

    const handleDisconnectButton = async () => {
        await http.post(`/api/client/extensions/minecraftpluginmanager/servers/${uuid}/minecraft-plugins/disconnect`);
        mutate();
    };
    ...
    useEffect(() => {
        http.get('https://piston-meta.mojang.com/mc/game/version_manifest_v2.json', {
            ...
            ({data}) => setMinecraftVersions(data.versions.filter(
                (v: { id: string; type: string }) => v.type === 'release'
